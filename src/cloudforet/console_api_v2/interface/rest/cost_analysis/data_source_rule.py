...
class DataSourceRule(BaseAPI):
    ...
    async def create(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.DataSourceRule.create'
    ...
    async def update(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.DataSourceRule.update'
            return proxy_service.dispatch_api(params)

    @router.post('/change-order')
    @exception_handler
    async def change_order(self, request: Request, body: dict = Body()):
        params, metadata = await self.parse_request(request, self.token.credentials)

        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.DataSourceRule.change_order'
            return proxy_service.dispatch_api(params)
    ...
    async def delete(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.DataSourceRule.delete'
            return proxy_service.dispatch_api(params)
    ...
    async def get(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.DataSourceRule.get'
            return proxy_service.dispatch_api(params)
    ...
    async def list(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.DataSourceRule.list'
            return proxy_service.dispatch_api(params)
    ...
    async def stat(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.DataSourceRule.stat'
            return proxy_service.dispatch_api(params)
