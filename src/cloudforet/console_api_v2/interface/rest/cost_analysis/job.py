...
class Job(BaseAPI):
    ...
    @router.post('/cancel')
    @exception_handler
    async def cancel(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.Job.cancel'
    ...
    async def get(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.Job.get'
            return proxy_service.dispatch_api(params)
    ...
    async def list(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.Job.list'
            return proxy_service.dispatch_api(params)
    ...
    async def stat(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.Job.stat'
            return proxy_service.dispatch_api(params)
