...
@cbv(router)
class ExchangeRate(BaseAPI):
    token: HTTPAuthorizationCredentials = Depends(_AUTH_SCHEME)
    service = 'console-api'

    @router.post('/set')
    @exception_handler
    async def set(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.ExchangeRate.set'
            return proxy_service.dispatch_api(params)

    @router.post('/reset')
    @exception_handler
    async def reset(self, request: Request, body: dict = Body()):
        params, metadata = await self.parse_request(request, self.token.credentials)

        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.ExchangeRate.reset'
            return proxy_service.dispatch_api(params)
    ...
    async def get(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.ExchangeRate.get'
            return proxy_service.dispatch_api(params)
    ...
    async def list(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'cost_analysis.ExchangeRate.list'
            return proxy_service.dispatch_api(params)
