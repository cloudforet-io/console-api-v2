...
@cbv(router)
class CloudServiceQuerySet(BaseAPI):
    ...
    async def create(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'inventory.CloudServiceQuerySet.create'
    ...
    async def delete(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'inventory.CloudServiceQuerySet.update'
            return proxy_service.dispatch_api(params)
    ...
    async def delete(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'inventory.CloudServiceQuerySet.delete'
            return proxy_service.dispatch_api(params)

    @router.post('/run')
    @exception_handler
    async def run(self, request: Request, body: dict = Body()):
        params, metadata = await self.parse_request(request, self.token.credentials)

        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'inventory.CloudServiceQuerySet.run'
            return proxy_service.dispatch_api(params)
    ...
    async def enable(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'inventory.CloudServiceQuerySet.enable'
            return proxy_service.dispatch_api(params)
    ...
    async def disable(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'inventory.CloudServiceQuerySet.disable'
            return proxy_service.dispatch_api(params)
    ...
    async def get(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'inventory.CloudServiceQuerySet.get'
            return proxy_service.dispatch_api(params)
    ...
    async def list(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'inventory.CloudServiceQuerySet.list'
            return proxy_service.dispatch_api(params)
    ...
    async def stat(self, request: Request, body: dict = Body()):
        ...
        with self.locator.get_service(ProxyService, metadata) as proxy_service:
            params['grpc_method'] = 'inventory.CloudServiceQuerySet.stat'
            return proxy_service.dispatch_api(params)
